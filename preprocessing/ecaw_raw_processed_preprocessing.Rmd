...
However, these responses will be included in an exploratory analysis that will be published in supplementary materials D. So we save them separately before exclusion.
...
processed_all_codebook <-
  ...
  mutate(
    has_missing = "True if respondent has any missing responses.",
    duration_in_mins = "Time spent from opening the survey to closing it in minutes."
  )
...
At the end of the exclusion we had `r nrow(raw)` responses remaining for the analysis. There were `r filter(raw, has_missing) %>% nrow()` respondents who had at least one missing value in the main questions of interests.
