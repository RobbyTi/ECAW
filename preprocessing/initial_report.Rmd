...
---
TODO: subset by concerned number of studies and current methods
```{r setup, include=FALSE}
# General rmarkdown rendering setting
...
library(likert)
library(viridis)
library(scales) # fto display percentages
...
# remove surveys where someone opened it, but didn't respond to any questions
raw <- raw %>% filter(progress!=0)
# number of respondents
nResponse <- raw %>% nrow()

# Filter for complete responses (I'm am maintaining the variable name "raw" only because it is used throughout the rest of the code)
raw <- raw %>% filter(finished)
#number of completed surveys
nResponseComplete <- raw %>% nrow()

invited <- 1148 # Number provided from Kyara at ALSPAC
bounced <- 54 # Number provided from Kyara at ALSPAC
received <- invited - bounced
```

# Response rate

We invited the ALSPAC mailing list to participate, which included `r invited` email addresses, of which `r bounced` bounced, leaving `r received` emails. The survey was completed `r nResponseComplete` times and partially completed `r nResponse - nResponseComplete` times. This gives a response rate for completed surveys of `r percent(nResponseComplete / received)` and a response rate for any questions `r percent(nResponse / received)`.

The median time taken for complete survey responses was `r (median(raw$duration_in_seconds) / 60) %>% round(1)` minutes (IQR: `r (quantile(raw$duration_in_seconds, .25) / 60) %>% round(1)` - `r (quantile(raw$duration_in_seconds, .75) / 60) %>% round(1)`)
...
# Prepearing data for Likert-scale plotting
...
# Creating text labels for reporting the count of non-likert response levels on the Likert-scale plot 
