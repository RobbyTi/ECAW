...
likert_plot_odd <- function(data, title, questions, levels, n, levels_to_drop, positive_side = c("lower", "upper"), limits = c(-0.5, 1), text_push = 0.1, legend_row = 1) {
  ...
  # Save color palette
  if(length(unique(likert_plot_data$levels_int)) == 7) {
    color_palette <- 
      data.frame(
        colors = c("#CD7F32", "#D5A16D", "#DDC3A9", "grey90", "#99D0CD", "#4EBCB5", "#03A89E"),
        levels_int = 1:7)
  } else if(length(unique(likert_plot_data$levels_int)) == 5) {
    color_palette <- 
      data.frame(
        colors = c("#D5A16D", "#DDC3A9", "grey90", "#99D0CD", "#4EBCB5"),
        levels_int = 1:5)
  } else if(length(unique(likert_plot_data$levels_int)) == 3) {
    color_palette <- 
      data.frame(
  ...
  # Create the plot
  fill_labels <- distinct(likert_plot_data, {{levels}}) %>% pull({{levels}})
